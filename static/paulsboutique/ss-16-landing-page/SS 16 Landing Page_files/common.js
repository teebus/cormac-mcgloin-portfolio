...
function _showFastCart(url) {
	...
	if(document.URL.indexOf('categories')!==-1) {
		$.post(url,function(data){
			try {
				reDoBag();
				$('#proddetails').html('').trigger('reveal:close');
	...
	//###################
	//## straight to mini cart
	$.post(url,function(data){
		try {
			reDoBag();
			$('html,body').animate({scrollTop: 0}, 0);
			if($(window).width()<=480){
				$('#mobileadd').slideDown(function(){
					setTimeout(function(){ $('#mobileadd').slideUp(); }, 8000);
				});
			} else {
				$('#Bag').addClass('hoverme');
			}
			if($('#crosssell').hasClass('showcrosssell')){
				$('#crosssell').reveal();
			}
		} catch (err) {}
	});	
...
function postCategoryLoad(){
    //Category view product details hover
    $(".catproddiv").hover(
        function () {
            $(this).find(".catvProdDetailPop").show();
        }, 
        function () {
            $(this).find(".catvProdDetailPop").hide();
        }
    );
}
